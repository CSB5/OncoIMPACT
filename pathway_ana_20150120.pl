...
my ( $step, $data_dir, $fraction_real_sample_used_parameter_inferance,
	$nb_thread, $network_type, $script_dir )
  = @ARGV;
...
#max should not have a sample median diff gene smaller than 0.01 of the gene with an expression
	...
	if ( !-d $test_param_dir ) {
		...
		print STDOUT "Parameter Estimation\n";
...
#cat  ovary/RANDOM_EXPRESSION_GENE/RANDOM_[0-9]/*/Genelist_Status_mut_impact_1000000_1000000.txt | awk '{if($2 != 0) print $0}' > ovary_random.dat
#R --vanilla < density.R
		...
		$exe = "tar -zcf $test_param_dir 2> /dev/null";
		run_exe($exe);    #<STDIN>;
	}
}
...
if ($RUN_DYS_SIGNIFICANCE) {
    ...
    print STDOUT "Phenotype Significance\n";
...
# $exe = "$module_inference_path DRIVER_SAMPLE $network_type $res_dir/exp_gene_freq_pvalue.dat 0.05 $best_depth_value $best_hub_value $res_dir/FINAL_MODULE_SAMPLE.dat $data_dir,$res_dir/MODULE.dat";
#run_exe($exe);
